...
- - P.S. Также предусмотрено скачивание архива во вкладке "Release" --> tags --> release 1.0 (ZIP)
...
![page](material/main_page.png)

_______

# Как работает программа

В данной программе используются два алгоритма — алгоритм Уоршелла и алгоритм Дейкстры. Вот как они работают:

## Алгоритм Уоршелла (транзитивное замыкание)
Работает с булевой матрицей смежности графа (значения 0 или 1, где 1 означает наличие ребра).

#### Цель — вычислить транзитивное замыкание, то есть определить достижимость вершины j из вершины i через цепочку ребер.

- Для каждой тройки вершин (i, j, k) проверяется, существует ли путь из i в k и из k в j. Если да, то устанавливается достижимость из i в j.

- Итогом является матрица, где элементы показывают, достижима ли вершина j из вершины i.

- Используется трёхуровневый вложенный цикл, чтобы учесть все промежуточные вершины для обновления матрицы.

- Применяется для анализа связности графа и нахождения всех достижимых пар вершин.

## Алгоритм Дейкстры (поиск кратчайшего пути)
Идёт работа с графом при неотрицательных весах ребер.

#### Задача — найти кратчайший путь от одной начальной вершины до конечной.

- Программа использует жадный подход, последовательно выбирая вершину с минимальным текущим расстоянием и обновляя расстояния до соседних вершин.

- Итогом является список вершин, составляющих кратчайший путь, и сумма весов по этому пути.

- Обеспечивается эффективный поиск, если веса ребер неотрицательны.
